...
$balance[ref]=0.00;
$sql=$Db1->query("SELECT SUM(upline_earnings) AS total FROM user WHERE refered='$user[username]'");
$totalrefearn=$Db1->fetch_array($sql);
$balance[ref]+=$totalrefearn[total];

$clicks[ref]=0;
$sql=$Db1->query("SELECT SUM(clicksref) AS total FROM user WHERE refered='$user[username]'");
	...
	for($x=0; $dluser=$Db1->fetch_array($sql); $x++) {
		...
		$downline.="
				<tr class=\"tableHL2\">
					...
					<td>\$$dluser[upline_earnings]</td>
					".iif($settings["ref_levels"] >= 2, "<td align=\"center\">$dluser[referrals1]</td>")."
					".iif($settings["ref_levels"] >= 3, "<td align=\"center\">$dluser[referrals2]</td>")."
					".iif($settings["ref_levels"] >= 4, "<td align=\"center\">$dluser[referrals3]</td>")."
					".iif($settings["ref_levels"] >= 5, "<td align=\"center\">$dluser[referrals4]</td>")."
					<td>$dluser[clicksref]</td>
					<td align=\"center\">".iif($dluser[last_act]!="", date('l, F j Y', @mktime(0,0,$dluser[last_act],1,1,1970)), "")."</td>
					<td align=\"center\">
					".iif($dluser[refstat] == 0,"<strong title=\"Referred Referral\">R</strong>")."
					".iif($dluser[refstat] == 1,"<strong title=\"Purchased Referral\">P</strong>")."
					".iif($dluser[refstat] == 2,"<strong title=\"Shifted Referral\">S</strong>")."
					".iif($dluser[refstat] == 3,"<strong title=\"Admin Assigned Referral\">A</strong>")."
					</td>
				</tr>
		";
	}
...
<form  name=\"frm\" action=\"admin.php?view=admin&ac=edit_user&action=shiftDL&id=$id&direct=$direct&start=$start&type=$type&orderby=$orderby&".iif($search==1,"search=1&search_str=$search_str&search_by=$search_by&")."".$url_variables."\" method=\"post\" onsubmit=\"return validate2()\">
			<table cellspacing=\"1\" cellpadding=\"1\" border=0 width=\"100%\">
				...
				<tr class=\"tableHL1\">
					<td align=\"left\" colspan=7><b>Totals</b></td>
				</tr>
