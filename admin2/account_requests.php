...
if($action == "cancel") {
	...
	$request=$Db1->fetch_array($sql);
		...
		$Db1->sql_close();
		header("Location: admin.php?view=admin&ac=account_requests&account=$account&".$url_variables."");
	}
...
$sql=$Db1->query("SELECT requests.*, withdraw_options.title FROM requests, withdraw_options WHERE requests.account='$account' and withdraw_options.id=requests.accounttype ORDER BY requests.dsub DESC");
if($Db1->num_rows() != 0) {
	while($temp=$Db1->fetch_array($sql)) {
		...
		<tr>
			...
			<td>$cursym $temp[amount]</td>
				<td><a href=\"admin.php?view=admin&ac=account_requests&action=markpaid&id=$temp[id]&account=$account&".$url_variables."\" onclick=\"return confirm('Are You Sure You Want To Mark This Paid?')\">Mark Paid</a></td>
			<td><a href=\"admin.php?view=admin&ac=account_requests&action=cancel&id=$temp[id]&account=$account&".$url_variables."\" onclick=\"return confirm('Are You Sure You Want To Cancel This?')\">Cancel</a></td>
			<td><a href=\"admin.php?view=admin&ac=account_requests&action=delete&id=$temp[id]&account=$account&".$url_variables."\" onclick=\"return confirm('Are You Sure You Want To Delete This?')\">Delete</a></td>
		</tr>
		";
	}
}
...
$sql=$Db1->query("SELECT * FROM payment_history WHERE account='$account' ORDER BY dsub DESC");
if($Db1->num_rows() != 0) {
	while($temp=$Db1->fetch_array($sql)) {
		$historylist.="
		<tr>
...
<form action=\"admin.php?view=admin&ac=admin&ac=account_requests&".$url_variables."\" method=\"post\">
	Account: <input type=\"text\" name=\"account\" value=\"$account\"> <input type=\"submit\" value=\"Load\"><br />
</form>
