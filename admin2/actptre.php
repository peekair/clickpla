...
if($action == "update") {
	for($x=0; $x<count($ssite); $x++) {
		$Db1->query("UPDATE emails SET active='$ssite[$x]' WHERE id='$siteid[$x]'");
	}
}
...
for($x=0; $site=$Db1->fetch_array($sql); $x++) {
	$list.="
		<tr onclick=\"this.bgColor='lightyellow'\">
			<td><a href=\"frame.php?id=$site[id]&type=actptre&openid=$x\" target=\"_blank\"><b>$site[title]</b><br />$site[target]</a><br />Credits: $site[credits]</td>
			<td>
...
<form action=\"admin.php?view=admin&ac=actptre&action=update&".$url_variables."\" method=\"post\" name=\"actform\">
<table width=\"100%\">
	<tr>
