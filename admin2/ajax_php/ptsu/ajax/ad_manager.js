...
///*				Edit Ad Functions			 */
	...
	function do_edit_ad(id){
		...
		var ajax = new Ajax.Updater('returnOut',"adminAjax.php?resource=ptsu&action=do_edit&id="+id+"&"+url_variables, {method:'post', postBody:formElements, evalScripts:true, asynchronous:true});
	}
	
	function edit_done() {
		setLoading('loading_alert', 0);
	//	document.getElementById("edit_ad").style.height=400;
		document.getElementById("edit_ad_message").innerHTML='Your changes have been saved!';
	//	jQuery(document.getElementById("edit_ad_message")).slideDown("slow");
		jQuery(document.getElementById("edit_ad_message")).show("slow");
	}
...
///*				New Ad Functions			 */
	...
	function create_done() {
		setLoading('loading_alert', 0);
		document.getElementById("new_ad_message").innerHTML='Your ad has been created!';
		jQuery(document.getElementById("new_ad_message")).slideDown("slow");
	}
...
///*				Delete Functions			 */
	function delete_ad(id) {
		if(confirm("Are you sure you want to delete this ad?")) {
			setLoading('loading_alert', 1);
			var ajax = new Ajax.Updater('returnOut',"adminAjax.php?resource=ptsu&action=delete_ad&id="+id+"&"+url_variables, {method:'post', evalScripts:true, asynchronous:true});
		}
	}
	function delete_done() {
		setLoading('loading_alert', 0);
		document.getElementById("edit_ad").innerHTML='<div align="center" style="padding: 10 0 0 0px">You must select an ad to edit from the manage ads tab.</div>';
		refresh_manager();
...
///*				Approve Ads Functions			 */
	...
	function approve_ads_sm(targetobj) {
		jQuery(document.getElementById(targetobj)).show("slow");
	}
	...
	function approve_done(contId) {
		setLoading('loading_alert', 0);
		jQuery(document.getElementById("approve_ad_main"+contId)).hide("slow");
	}
...
///*				Main Manager Functions			 */
	...
	function dosearch() {
		document.getElementById("manage_ads").innerHTML='<div style="height: 350px"></div>';
		manager_start = 0;
		manager_epp = 10;
		manager_sortType = '';
		manager_sortOrder = '';
	...
	function loadFile(url, containerid, grabtab){
	...
	function approve_signup(id,approval) {
		setLoading('loading_alert', 1);
		var ajax = new Ajax.Updater('returnOut',"adminAjax.php?resource=ptsu&action=do_approve_signup&id="+id+"&approve="+approval+"&"+url_variables, {method:'post', evalScripts:true, asynchronous:true});
	}
	function approve_done_signup(contId) {
		setLoading('loading_alert', 0);
		jQuery(document.getElementById("approve_signup_main"+contId)).hide("slow");
	}