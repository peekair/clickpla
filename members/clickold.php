	...
	for($x=0; $ad=$Db1->fetch_array($sql); $x++) {
		if(findclick($clickHistory, $ad['id']) == 0) {
			...
			<tr  id=\"col$x\" ".iif($ad[bgcolor]!="","style=\"background-color: $ad[bgcolor] !important;\" class=\"adHighlighted\" ","class=\"zebra".(($x%2==1)?"1":"2")."\"").">
				...
				<a href=\"gpt.php?v=entry&type=ptc&id=$ad[id]&".$url_variables."\" target=\"_blank\" onclick=\"clicked1('col$x')\">".parse_link(ucwords(strtolower(stripslashes($ad[title]))))."</a><br> ".iif($ad['targetban']!="","<img src=\"$ad[targetban]\"width=\"300\" border=\"0\">")."".iif($ad['targetban']==0 ,".")."
