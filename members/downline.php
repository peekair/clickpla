	...
	if(count($select) != 0) {
		for($x=0; $x<count($select); $x++) {
			...
			if(($tempuser[username] == $touser) && ($settings[allow_self_ref] != 1)) {
				$thelist.="<li><font color=\"red\">$tempuser[username]</font> : Cannot shift user to their own downline!";
			}
			else {
		...
		$includes[content]="The following referrals were shifted to the account: $touser<br /><br />$thelist";
	}
}
else {
...
if($user[type] == 1) $upstats="Yes";
		...
		$downline.="
				<tr class=\"tableHL2\">
						".iif($settings[shifteron]==1,"<td>	
						<input type=\"checkbox\" value=\"$user[userid]\" name=\"select[]\">
					</td>")."
					<td>$user[username]   ".iif(!$user[coclicks]  <$clicksleft,"-<a href=\"index.php?view=account&ac=downlinebonus&username=".$user["username"]."&".$url_variables."\">Claim bonus</a>")." </td>
					<td>$upstats</td>
...
<form  name=\"frm\" action=\"index.php?view=account&ac=downline&action=move&".$url_variables."\" method=\"post\" onsubmit=\"return validate2()\">
			<table cellspacing=\"1\" cellpadding=\"1\" border=0 width=\"100%\">
				<tr class=\"tableHL1\">
					".iif($settings[shifteron]==1,"<td><input type=\"checkbox\" value=\"1\" name=\"allbox\" onClick=\"CA()\"></td>")."
					<td><b>Level 1</b></td>
					<td><b>Premium</b></td>
					<td><b>Earnings</b></td>
...
<br />Please Note :<BR> These Totals Are Not Overall Totals. They Only Show Your Active Referrals<br> Not Members Who Have Been Deleted or Shifted From Your Account<BR><BR>
<div align=\"left\">
