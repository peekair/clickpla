...
$correct_code=1;
	...
	if(($correct_code==0) && ($settings[login_route] == 1)) {
		$msg="<div align=\"center\"><b>Login Error!</b></div><br />You did not enter the correct captcha! Please try again.";
	}
	elseif(($form_user != "") && ($form_pwd != "")) {
		$sql=$Db1->query("SELECT userid, password, username,suspended, suspendTime, suspendMsg,verified, permission, lockout, warnings FROM user WHERE username='$form_user'");
		if($Db1->num_rows() != 0) {
...
$Db1->query("UPDATE user SET country='$co' WHERE username='$form_user'");
                    ...
                    if($userinfo[warnings] >= $settings[nomfw]) {                           
                                     $LOGGED_IN=false;
                                     $msg = "<div align=\"center\"><b>Login Error!</b></div><br>Your account has had $userinfo[warnings] warnings, you are now suspended.";
                            }
                            
                                 else if($userinfo[suspended]==1) {
					$LOGGED_IN=false;
...
if(($form_user != "") && ($form_pwd != "")) {
		...
		if($Db1->num_rows() != 0) {
			...
			if(($userinfo[username] == $form_user) && ($userinfo[password] == md5($form_pwd))) {
				...
				else if($userinfo[warnings] >= $settings[nomfw]) {                           
                                     $LOGGED_IN=false;
                                     $msg = "<div align=\"center\"><b>Login Error!</b></div><br>Your account has had $userinfo[warnings] warnings, you are now suspended.";
                            }
...
<td class=\"tableHL1\" >
            ...
            <div align=\"center\"><input type=\"submit\" value=\"Login\" class=\"login_submit\"></div>
          </td></tr>
