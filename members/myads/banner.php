...
//#        taken if neccessary.                                                    #
//#########################################//   
$includes[title]="Manage Banner Ads";
...
if($action == "add") {
	...
	if((!isset($title)) || ($title=="")) {
		...
		$error_msg="You must enter a valid banner URL!";
	}
		...
		$error_msg="Your Banner Must Not Violate The Terms!";
	}
	else if(is_ad_blocked($target)) {
		$error_msg="Error 382 was returned. Please contact support!";
	}
	else {
		$sql=$Db1->query("INSERT INTO banners SET title='$title', daily_limit='".addslashes($dlimit)."', dsub='".time()."', username='$username', banner='$banner', target='$target'");
		$Db1->sql_close();
                mail($settings['admin_email'], "New banner ad at {$settings['site_title']}", "Hello admin,\n\nThere is a new banner ad at {$settings['site_title']}.");

		header("Location: index.php?view=account&ac=myads&adtype=banner&".$url_variables."");
	}
}

$sql=$Db1->query("SELECT * FROM banners WHERE username='$username'");
if($Db1->num_rows() != 0) {
	while($ad=$Db1->fetch_array($sql)) {
	$bannerads.="
	<tr>
		<td widtd=150>&nbsp; <a href=\"index.php?view=account&ac=view_banner&id=$ad[id]&".$url_variables."\">".stripslashes($ad[title])."</a></b></td>
		<td>&nbsp; $ad[credits]</td>
		<td>&nbsp; $ad[views]</td>
		...
		<td align=\"center\">
			<a href=\"index.php?view=account&ac=add_credits_banner&id=$ad[id]&".$url_variables."\">Add Credits</a>
			".iif($ad[forbid_retract]==0 && ($settings[allow_retract] == 1),"&nbsp;&nbsp;&nbsp;<a href=\"index.php?view=account&ac=retract_credits_banner&id=$ad[id]&".$url_variables."\">Retract</a>")."
			</td>
				<td align=\"center\">
	...
	$bannerads="
		<tr>
			<td colspan=7 align=\"center\">You Have Not Added Any Banner Ads!</td>
		</tr>
	";
}
...
<form action=\"index.php?view=account&ac=myads&adtype=banner&action=add&".$url_variables."\" method=\"post\" onSubmit=\"submitonce(this)\">
