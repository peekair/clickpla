...
if($action == "add") {
	...
	$target=stripslashes(str_replace("'","",$target));
	$ad=stripslashes(str_replace("\"","",$ad));
	$ad=stripslashes(str_replace("'","",$ad));
	...
	$sql=$Db1->query("INSERT INTO ptrads SET title='$title', country='".addslashes($country)."', upgrade='".addslashes($premOnly)."', daily_limit='".addslashes($dlimit)."', ad='$ad', dsub='".time()."', username='$username', target='$target', active='".iif($settings[ptra_approve]==1,"0","1")."'");
		$Db1->sql_close();
                mail($settings['admin_email'], "New PTR ad at {$settings['site_title']}", "Hello admin,\n\nThere is a new PTR ad at {$settings['site_title']}.");
...
$sql=$Db1->query("SELECT * FROM ptrads WHERE username='$username'");
if($Db1->num_rows() != 0) {
	...
	<tr>
		...
		<td>&nbsp; <a href=\"index.php?view=account&ac=view_ptrad&id=$ad[id]&".$url_variables."\" title=\"".ucwords(strtolower(stripslashes($ad[title])))."\">".parse_link(ucwords(strtolower(stripslashes($ad[title]))),30)."</a></b></td>
		<td>&nbsp; $ad[credits]</td>
		<td>&nbsp; $ad[views]</td>
...
<td align=\"center\">
		...
		<td align=\"center\">
			<a href=\"index.php?view=account&ac=add_credits_ptrad&id=$ad[id]&".$url_variables."\">Add</a>
			".iif($ad[forbid_retract]==0 && $ad['class']!="P" && ($settings[allow_retract] == 1),"&nbsp;&nbsp;&nbsp;<a href=\"index.php?view=account&ac=retract_credits_ptrad&id=$ad[id]&".$url_variables."\">Retract</a>")."
			</td>
	</tr>
	";
	}
	...
	$linkads="
		<tr class=\"tableHL2\">
			<td colspan=6 align=\"center\">There Are No PTR Ads On Your Account</td>
		</tr>
	";
}
...
<table class=\"ptcList\">
	<tr>
		...
		<th>Class</th> 
                <th>Status</th>
		<th align=\"center\">Credit Actions</th>
	</tr>
	$linkads
</table>
...
<form action=\"index.php?view=account&ac=myads&adtype=ptrads&action=add&".$url_variables."\" method=\"post\" onSubmit=\"submitonce(this)\">
