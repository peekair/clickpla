...
//#        taken if neccessary.                                                    #
//#########################################//   
$includes[title]="Manage Click Exchange Sites";
...
if($action == "add") {
	...
	if($title == "") {
		...
		$sql=$Db1->query("INSERT INTO xsites SET title='$title', country='".addslashes($country)."', dsub='".time()."', upgrade='".addslashes($premOnly)."', daily_limit='".addslashes($dlimit)."', username='$username', target='$target', active='".iif($settings[ce_approve]==1,"0","1")."'");
		$Db1->sql_close();
               mail($settings['admin_email'], "New click exchange ad at {$settings['site_title']}", "Hello admin,\n\nThere is a new click exchange ad at {$settings['site_title']}.");
...
$sql=$Db1->query("SELECT * FROM xsites WHERE username='$username'");
if($Db1->num_rows() != 0) {
	while($ad=$Db1->fetch_array($sql)) {
	$linkads.="
	<tr>
		<td>&nbsp; <a href=\"index.php?view=account&ac=view_xsite&id=$ad[id]&".$url_variables."\" title=\"".ucwords(strtolower(stripslashes($ad[title])))."\">".parse_link(ucwords(strtolower(stripslashes($ad[title]))),30)."</a></b></td>
		<td>&nbsp; $ad[credits]</td>
		<td>&nbsp; $ad[views]</td>
...
<td>&nbsp; $ad[decline]</td>
		<td align=\"center\">
			<a href=\"index.php?view=account&ac=add_credits_xsite&id=$ad[id]&".$url_variables."\">Add</a>
			".iif($ad[forbid_retract]==0 && ($settings[allow_retract] == 1),"&nbsp;&nbsp;&nbsp;<a href=\"index.php?view=account&ac=retract_credits_xsite&id=$ad[id]&".$url_variables."\">Retract</a>")."
			</td>
	</tr>
	";
	}
	...
	$linkads="
		<tr class=\"tableHL2\">
			<td colspan=6 align=\"center\">There Are No Click Exchange Sites On Your Account</td>
		</tr>
	";
}
...
<form action=\"index.php?view=account&ac=myads&adtype=xsites&action=add&".$url_variables."\" method=\"post\" onSubmit=\"submitonce(this)\">
<div style=\" width: 300px;\">
<b>New Click Exchange Site</b>
<table>
	<tr>
		<td>Title: </td>
