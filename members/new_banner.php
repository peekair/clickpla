...
//#        taken if neccessary.                                                    #
//#########################################//   
$includes[title]="Add A New Banner";
//**S**//
if($action == "add") {
	...
	else if((!isset($banner)) || ($banner=="") || ($banner=="http://")) {
		$error_msg="You must enter a valid banner URL!";
	}
	else {
		$sql=$Db1->query("INSERT INTO banners SET title='$title', dsub='".time()."', username='$username', banner='$banner', target='$target'");
		$Db1->sql_close();
...
//**E**//
$includes[content]="".iif($settings[adminapproved] ==1,"
...
<div align=\"center\">
<form action=\"index.php?view=account&ac=new_banner&action=add&".$url_variables."\" method=\"post\" onSubmit=\"submitonce(this)\">
<table>
	<tr>
		<td>Title: </td>
