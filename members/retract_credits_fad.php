...
//#        taken if neccessary.                                                    #
//#########################################//   
$includes[title]="Retract Featured Ad Credits";
//**S**//
...
if($action == "retract") {
	if($credits < 1) {
		...
		$sql=$Db1->query("UPDATE fads SET credits=credits-$credits WHERE id='$id'");
		$sql=$Db1->query("UPDATE user SET fad_credits=fad_credits+$credits WHERE username='$username'");
		$Db1->sql_close();
...
if(($thisad['class'] != "P") || ($thisad[forbid_retract] != 1)) {
	...
	<br />
<div align=\"center\">
<form action=\"index.php?view=account&ac=retract_credits_fad&action=retract&".$url_variables."\" method=\"post\">
