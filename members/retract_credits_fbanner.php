...
//#        taken if neccessary.                                                    #
//#########################################//   
$includes[title]="Retract Featured Banner Credits";
//**S**//
...
if($action == "retract") {
	if($credits < 1) {
		...
		$sql=$Db1->query("UPDATE fbanners SET credits=credits-$credits WHERE id='$id'");
		$sql=$Db1->query("UPDATE user SET fbanner_credits=fbanner_credits+$credits WHERE username='$username'");
		$Db1->sql_close();
...
if(($thisad['class'] != "P") || ($thisad[forbid_retract] != 1)) {
	...
	<br />
<div align=\"center\">
<form action=\"index.php?view=account&ac=retract_credits_fbanner&action=retract&".$url_variables."\" method=\"post\">
<input type=\"hidden\" value=\"$id\" name=\"id\">
<a href=\"$thisad[target]\" target=\"_blank\"><img src=\"$thisad[banner]\" border=0></a>
<table>
	<tr>
		<td>Title: </td>
		<td>$thisad[title]</td>
...
";
	...
	$includes[content]="You cannot retract the credits from this banner!";
}
//**E**//
