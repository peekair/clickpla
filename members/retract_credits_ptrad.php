...
//#        taken if neccessary.                                                    #
//#########################################//   
$includes[title]="Retract PTR Credits";
//**VS**//$setting[ptra]//**VE**//
//**S**//
...
if($action == "retract") {
	if($credits < 1) {
		...
		$sql=$Db1->query("UPDATE ptrads SET credits=credits-$credits WHERE id='$id'");
		$sql=$Db1->query("UPDATE user SET ptra_credits=ptra_credits+".($credits*$settings['ptr_'.strtolower($thisad['class']).'_credit_ratio'])." WHERE username='$username'");
		$Db1->sql_close();
...
if(($thisad['class'] != "P") || ($thisad[forbid_retract] != 1)) {
	...
	<br />
<div align=\"center\">
<form action=\"index.php?view=account&ac=retract_credits_ptrad&action=retract&".$url_variables."\" method=\"post\">
...
";
	...
	$includes[content]="You cannot retract the credits from this ad!";
}
//**E**//
