...
//#        taken if neccessary.                                                    #
//#########################################//   
$includes[title]="Retract Paid Offer Credits";
//**VS**//$setting[ptsu]//**VE**//
//**S**//
...
if($action == "retract") {
	if($credits < 1) {
		...
		$sql=$Db1->query("UPDATE ptsuads SET credits=credits-$credits WHERE id='$id'");
		$sql=$Db1->query("UPDATE user SET ptsu_credits=ptsu_credits+$credits WHERE username='$username'");
		$Db1->sql_close();
...
if(($thisad['class'] != "P") || ($thisad[forbid_retract] != 1)) {
	...
	<br />
<div align=\"center\">
<form action=\"index.php?view=account&ac=retract_credits_ptsu&action=retract&".$url_variables."\" method=\"post\">
