...
function get_banner() {
    ...
    if($banner[banner] != "") {
        $banner[html] = "<a href='bannerclick.php?id=$banner[id]' target='_blank'><img alt='banner' src='$banner[banner]' border='0' width='468' height='60'></a>\n";
    }
    return $banner[html];
}
...
function get_contenta($title="",$content="", $width="100%", $height="100%") {
return "
...
<script>
    ...
    var games = ".iif(SETTING_GAMES==true,"1","0").";
</script>
...
if($pagefad[title] != "") {
	$sql=$Db1->query("SELECT * FROM fads WHERE credits>=1 and (daily_limit>views_today or daily_limit=0) ".iif($LOGGED_IN==true," and username!='$username'")." order by rand() ".iif($settings[fads_show]!=0,"limit $settings[fads_show]")."");
	while($pagefad=$Db1->fetch_array($sql)) {
		$idzlist.=", $pagefad[id]";
		$fadslist.="<div style=\"padding-bottom:5px;\"><b><a href=\"fadclick.php?id=$pagefad[id]\" target=\"_blank\"><font color=\"$settings[fads_title]\">".ucwords(strtolower($pagefad[title]))."</font></a></b><br/><font color=\"$settings[fads_desc]\">".stripslashes(ucwords(strtolower($pagefad[description])))."</font></div>";
	}
	$sql=$Db1->query("UPDATE fads SET credits=credits-1, views=views+1 WHERE id IN (0".$idzlist.")");
	srand(time());
	$random = (rand()%2);
}
...
if($settings[flink_style] == 2) {
    ...
    while($temp=$Db1->fetch_array($sql)) {
        $idlist.=", $temp[id]";
        $flinklist.="<div".iif($temp[bgcolor]!=""," style=\"background-color: $temp[bgcolor]\"")."><a href=\"flinkclick.php?id=$temp[id]\" target=\"_blank\">".iif($temp[marquee]==1,"<marquee>".ucwords(strtolower($temp[title]))."</marquee>","$temp[title]")."</a></div><hr>";
    }
...
if($pollshow == 1) {
    ...
    if($Db1->num_rows() == 0) {
        ...
        <b>$settings[poll_title]</b><br />
        <small>
        ".iif($settings[poll_a1] != "", "<li>$settings[poll_a1] : $pollresults[1]</li><br />")."
        ".iif($settings[poll_a2] != "", "<li>$settings[poll_a2] : $pollresults[2]</li><br />")."
        ".iif($settings[poll_a3] != "", "<li>$settings[poll_a3] : $pollresults[3]</li><br />")."
        ".iif($settings[poll_a4] != "", "<li>$settings[poll_a4] : $pollresults[4]</li><br />")."
        ".iif($settings[poll_a5] != "", "<li>$settings[poll_a5] : $pollresults[5]</li><br />")."
        </small>
        ";
    }
...
if($fbannershow == 1) {
    ...
    if($pagebanner180[id] != "") {
        $areacontent.="<a href=\"fbannerclick.php?id=$pagebanner180[id]\" target=\"_blank\"><img src=\"$pagebanner180[banner]\" border=0\" ></a>";
    }
}
...
function fbanner(){
    ...
    $sql=$Db1->query("UPDATE fbanners SET credits=credits-1, views=views+1, views_today=views_today+1 WHERE id='$pagebanner180[id]'");
    if($pagebanner180['id'] != "") {
        return "<a href='fbannerclick.php?id=$pagebanner180[id]' target='_blank'><img alt='banner' src='$pagebanner180[banner]' border='0' width='180' height='100'></a>\n\n";
    }
}
...
function fbanner100(){
    ...
    $sql=$Db1->query("UPDATE fbanners SET credits=credits-1, views=views+1, views_today=views_today+1 WHERE id='$pagebanner180[id]'");
    if($pagebanner180['id'] != "") {
        return "<a href='fbannerclick.php?id=$pagebanner180[id]' target='_blank'><img alt='banner' src='$pagebanner180[banner]' border='0' width='100' height='100'></a>\n\n";
    }
}
...
if($settings[homepage_stats] == 1) {
    ...
    $sql=$Db1->query("SELECT SUM(amount) AS total, COUNT(amount) AS amount FROM payment_history WHERE status != 2");
    $totalpaid=$Db1->fetch_array($sql);
}
if($fbannershow == 1) {
	$sqlz=$Db1->query("SELECT * FROM fbanners WHERE credits>=1 and (daily_limit>views_today or daily_limit=0) ".iif($LOGGED_IN==true," and username!='$username'")." order by rand()");
	$pagebanner180=$Db1->fetch_array($sqlz);
	
	if($pagebanner180[id] != "") {
	$sqlz=$Db1->query("SELECT * FROM fbanners WHERE credits>=1 and (daily_limit>views_today or daily_limit=0) ".iif($LOGGED_IN==true," and username!='$username'")." order by rand() ".iif($settings[fbanner_show]!=0,"limit $settings[fbanner_show]")."");
	while($pagebanner180=$Db1->fetch_array($sqlz)) {
		$idbannerlist.=", $pagebanner180[id]";
		$fbannerlist.="<a href=\"fbannerclick.php?id=$pagebanner180[id]\" target=\"_blank\"><img src=\"$pagebanner180[banner]\" border=\"0\" width=\"$settings[fbanner_w]\" height=\"$settings[fbanner_h]\"></a>";
	}
	$sqlz=$Db1->query("UPDATE fbanners SET credits=credits-1, views=views+1, views_today=views_today+1 WHERE id IN (0".$idbannerlist.")");
    ...
    $homepagestats="
        <table border='0'  bgcolor=\"\" cellpadding=\"2\" cellspacing=\"2\" align=\"center\">
        <tr>
            <td align='center'> Members</td>
            <td align='center'>".number_format($totalmembers['total'])."</td>
        </tr>
        <tr>
            <td align='center'> New Today</td>
            <td align='center'>".number_format($todaystats['new_members'])."</td>
        </tr>
        <tr>
            <td align='center'>Hits Today</td>
            <td align='center'>".number_format($todaystats['hits'])." </td>
        </tr>
        <tr>
            <td align='center'> Online</td>
            <td align='center'>".number_format($total_online)."</td>
        </tr>
        <tr>
            <td align='center'> Payouts Made</td>
            <td align='center'> ".number_format($totalpaid['amount'])."</td>
        </tr>
        <tr>
            <td align='center'>Total Paid </td>
            <td align='center'>$".number_format(round($totalpaid['total']),2)."</td>
        </tr>
        </table>\n\n";
}
